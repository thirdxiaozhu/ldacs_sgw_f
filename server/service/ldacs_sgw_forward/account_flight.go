...
func (accountFlightService *AccontFlightService) CreateAccontFlight(accountFlight *ldacs_sgw_forward.AccontFlight) (err error) {
...
func (accountFlightService *AccontFlightService)DeleteAccontFlight(id string) (err error) {
	err = global.GVA_DB.Delete(&ldacs_sgw_forward.AccontFlight{},"id = ?",id).Error
	return err
}
...
func (accountFlightService *AccontFlightService)DeleteAccontFlightByIds(ids []string) (err error) {
	err = global.GVA_DB.Delete(&[]ldacs_sgw_forward.AccontFlight{},"id in ?",ids).Error
	return err
}

// UpdateAccontFlight 更新航班记录
// Author [piexlmax](https://github.com/piexlmax)
func (accountFlightService *AccontFlightService)UpdateAccontFlight(accountFlight ldacs_sgw_forward.AccontFlight) (err error) {
...
// Author [piexlmax](https://github.com/piexlmax)
func (accountFlightService *AccontFlightService)GetAccontFlight(id string) (accountFlight ldacs_sgw_forward.AccontFlight, err error) {
...
// Author [piexlmax](https://github.com/piexlmax)
func (accountFlightService *AccontFlightService)GetAccontFlightInfoList(info ldacs_sgw_forwardReq.AccontFlightSearch) (list []ldacs_sgw_forward.AccontFlight, total int64, err error) {
	limit := info.PageSize
	offset := info.PageSize * (info.Page - 1)
    // 创建db
	db := global.GVA_DB.Model(&ldacs_sgw_forward.AccontFlight{})
    var accountFlights []ldacs_sgw_forward.AccontFlight
    // 如果有条件搜索 下方会自动创建搜索语句
